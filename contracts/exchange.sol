...
contract Exchange is ERC20 {
    ...
    // Exchange is inheriting ERC20, because our exchange 
    // would keep track of Crypto Dev LP tokens.
    ...
    function addLiquidity(uint _amount) public payable returns(uint) {
        ...
        */
        if(cryptoDevTokenReserve == 0) {
            ...
            // Take the current ethBalanceand mint 'ethBalance' amount of LP tokens to the user.
            // 'liquidity' provided is equal to 'ethBalance' because this is the first time user
            // is adding 'Eth' to the contract, so whatever 'Eth' contract has is equal to the
            // one supplied by the user in the current 'addLiquidity' call.
            // 'liquidity' tokens that need to be minted to the user on 'addLiquidity' call
            // should always be proportional to the Eth specified by the user.
            ...
            /*
                ...
                need to be supplied to prevent any Large price impacts because of the
                additional liquidity
            */
            // EthReserve should be the current ethBalance subtracted by the value of ether sent
            ...
            // The amount of LP tokens that would be sent to the user should be proportional
            // to the liquidity of ether added by the user.
            // Ratio here to be maintained is ->
            // (LP tokens to be sent to the user (liquidity) / totalSupply of LP tokens in contract) = (Eth sent by the user) / (Eth reserve in the contract).
            // by some maths -> liquidity = (totatlSupply of LP tokens in contract * (Eth sent by the user)) / (Eth reserve in the contract).
    ...
    */
    function removeLiquidity(uint _amount) public returns (uint, uint) {
        ...
        // then some maths -> (Eth sent back to the user) = 
        ...
        // Burn the sent LP tokens from the user's wallet because they are already sent
        // to remove liquidity.
        _burn(msg.sender, _amount);
        // Transfer 'ethAmount' of Eth from the contract to the user's wallet
        payable(msg.sender).transfer(ethAmount);
        // Transfer 'cryptoDevTokenAmount' of Crypto Dev tokens from the contract
